...
type OperLogDTO struct {
	...
	//模块
	Module string `excel:"column:模块;index:1;dict:" json:"module" binding:"max=50"`
	//类型
	Type string `excel:"column:类型;index:2;dict:" json:"type" binding:"max=10"`
	//描述
	Description string `excel:"column:描述;index:3;dict:" json:"description" binding:"max=128"`
	//请求方法
	Method string `excel:"column:请求方法;index:4;dict:" json:"method" binding:"max=10"`
	//请求URL
	Url string `excel:"column:请求URL;index:5;dict:" json:"url" binding:"max=255"`
	//请求IP
	Ip string `excel:"column:请求IP;index:6;dict:" json:"ip" binding:"max=128"`
	//请求地点
	Location string `excel:"column:请求地点;index:7;dict:" json:"location" binding:"max=255"`
	//请求参数
	Payload string `excel:"column:请求参数;index:8;dict:" json:"payload" binding:"max=2000"`
	//响应数据
	Result string `excel:"column:响应数据;index:9;dict:" json:"result" binding:"max=2000"`
	//设备
	Device string `excel:"column:设备;index:10;dict:" json:"device" binding:"max=50"`
	//系统
	Os string `excel:"column:系统;index:11;dict:" json:"os" binding:"max=50"`
	//浏览器
	Browser string `excel:"column:浏览器;index:12;dict:" json:"browser" binding:"max=50"`
	//状态（0失败 1成功）
	Status int16 `excel:"column:状态（0失败 1成功）;index:13;dict:" json:"status"`
	//错误消息
	Error string `excel:"column:错误消息;index:14;dict:" json:"error" binding:"max=2000"`
	//耗时（毫秒）
	Time int64 `excel:"column:耗时（毫秒）;index:15;dict:" json:"time,string"`
	//用户编号
	UserId int64 `excel:"column:用户编号;index:16;dict:" json:"userId,string"`
	//用户名
	Username string `excel:"column:用户名;index:17;dict:" json:"username" binding:"max=50"`
	//操作时间
	OperAt time.Time `excel:"column:操作时间;index:18;dict:" json:"operAt"`
}
