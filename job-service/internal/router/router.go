...
func Setup(app *core.App) (*gin.Engine, error) {
	...
	v1 := r.Group("/api/v1/job")
	{
		...
		registerJobRoutes(v1, db, logWriter, cfg.Gen)
	}

	return r, nil
}

func registerJobRoutes(rg *gin.RouterGroup, db *gorm.DB, logWriter log_grpc.OperLogWriter, genCfg config.GenConfig) {
	...
	rg.GET("/page", middleware.HasPermission("job$list"), h.Page)
	rg.GET("/:id", middleware.HasPermission("job$query"), h.Get)
	rg.POST("", middleware.HasPermission("job$add"), localmiddleware.OperLog(logWriter, "Job-[定时任务]", consts.INSERT), h.Create)
	rg.PUT("/:id", middleware.HasPermission("job$edit"), localmiddleware.OperLog(logWriter, "Job-[定时任务]", consts.UPDATE), h.Update)
	rg.DELETE("/:ids", middleware.HasPermission("job$delete"), localmiddleware.OperLog(logWriter, "Job-[定时任务]", consts.DELETE), h.BatchDelete)
	// 手动触发
	rg.POST("/:id/trigger", middleware.HasPermission("job$trigger"), h.Trigger)
	// 启动/暂停
	rg.PUT("/:id/status", middleware.HasPermission("job$edit"), h.ChangeStatus)
